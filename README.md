...
2) Feature Removal
Certain features were removed due to redundancy, excessive missing values, or lack of predictive utility. Unique identifiers such as Loan ID and Borrower ID were removed as they serve only as unique identifiers and provide no meaningful information for predicting interest rates. Features with a high proportion of missing values, such as Loan Grade, Loan Subgrade, Home Ownership, Loan Purpose, Months Since Last Delinquency, and Months Since Last Public Record, were dropped because a large portion of their values were missing, making imputation unreliable. Some categorical features, such as Employer Name, Loan Title, and First Three Digits of Zip Code, had too many unique values with little standardization, making them unsuitable for predictive modeling.
3) Handling Missing Values
We applied different imputation techniques to maintain data consistency and avoid introducing biases. Missing values in categorical features such as Number of Payments, Years Employed, Annual Income, Income Verification, Loan Category, State, and Initial Listing Status were filled with the most frequent value (mode). Missing values in numerical features such as Debt-to-Income Ratio, Delinquencies in Last Two Years, Credit Inquiries in Last Six Months, Number of Open Credit Lines, Number of Derogatory Public Records, Total Revolving Balance, Revolving Line Utilization Rate, and Total Credit Lines were imputed using the median to minimize the influence of outliers. Essential feature completeness was ensured by removing rows missing values in crucial categorical features such as Number of Payments and Income Verification, since these variables are essential in determining loan terms and applicant credibility. Rows where the target variable Interest Rate was missing were removed from the training dataset to ensure the model only learns from complete records.
4) Data Type Conversions
Percentage and currency values such as Interest Rate, Loan Amount Requested, Loan Amount Funded, Investor-Funded Portion, and Revolving Utilization Rate were converted into numerical format after removing non-numeric characters such as percent, dollar signs, and commas. Date variables such as Loan Issued Date and Earliest Credit Line Opened were transformed into datetime format for potential feature engineering related to time-based trends.
...
Days From Last Loan: This feature captures the time gap between two key loan-related dates, providing insights into borrower behavior over time.
